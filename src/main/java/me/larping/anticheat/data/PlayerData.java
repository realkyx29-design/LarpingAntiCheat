...
public final class PlayerData {
    ...
    // ---- ESP honey-pot decoy approach tracking ----
    // decoy entityId -> {closest distance so far, consecutive approach ticks}
    private final java.util.Map<java.util.UUID, double[]> decoyApproach = new java.util.HashMap<>();

    /**
     * Records distance to a decoy. Returns true if the player is sustainedly
     * approaching (distance shrinking across several samples). State prunes
     * stale decoys automatically.
     */
    public boolean recordDecoyApproach(java.util.UUID decoyId, double dist) {
        long now = System.currentTimeMillis();
        double[] st = decoyApproach.computeIfAbsent(decoyId, k -> new double[]{Double.MAX_VALUE, 0, now});
        double prev = st[0];
        if (dist < prev - 0.15) {
            st[1] += 1;
        } else if (dist > prev + 2.0) {
            st[1] = Math.max(0, st[1] - 1); // moving away resets
        }
        st[0] = Math.min(prev, dist);
        st[2] = now;
        // prune stale entries
        decoyApproach.entrySet().removeIf(e -> now - e.getValue()[2] > 60_000L);
        return st[1] >= 6;
    }
    public void clearDecoyTracking() { decoyApproach.clear(); }
    ...
    /** The raw last knockback vector applied (for re-enforcement when resisted). */
    public double velX(){ return velX; }
    public double velY(){ return velY; }
    public double velZ(){ return velZ; }
    public long velocityAppliedMs(){ return velocityAppliedMs; }
